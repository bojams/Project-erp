...
const emptyRow = (): PurchaseItemRow => ({ product_id: '', quantity: '1', unit_price: '0' })

export function PurchaseFormModal({ open, onClose, onSave, suppliers, products, isLoading, errorMessage }: PurchaseFormModalProps) {
  ...
  const updateItem = (i: number, key: keyof PurchaseItemRow, value: string) => {
    setItems(prev => {
      const next = [...prev]
      ...
      if (key === 'product_id') {
        ...
        if (product) {
          next[i].unit_price = String(product.purchase_price)
        }
      }
      return next
    })
  ...
  const handleSave = () => {
    ...
    if (!supplierId) newErrors.supplier_id = 'Pemasok harus dipilih'
    ...
    <Dialog.Root open={open} onOpenChange={(e) => { if (!e.open) onClose() }} placement="center" size="lg">
      <Portal>
        ...
        <Dialog.Positioner>
          <Dialog.Content>
            <Dialog.Header>
              <Dialog.Title>Pembelian Baru</Dialog.Title>
            </Dialog.Header>
            <Dialog.Body>
              <VStack gap="4">
                {errorMessage && (
                ...
                <Grid templateColumns="repeat(2, 1fr)" gap="4" w="full">
                  <Field.Root invalid={!!errors.supplier_id}>
                    <Field.Label>Pemasok</Field.Label>
                    <NativeSelect.Root>
                    ...
                    <Field.ErrorText>{errors.supplier_id}</Field.ErrorText>
                  </Field.Root>
                          ...
                          <Field.Root invalid={!!errors[`items.${i}.product_id`]}>
                            <NativeSelect.Root>
                              <NativeSelect.Field value={row.product_id} onChange={(e) => updateItem(i, 'product_id', e.target.value)}>
                                ...
                                {products.map((p) => (
                                  <option key={p.id} value={p.id}>{p.name} (Rp {p.purchase_price.toLocaleString()})</option>
                                ))}
                              </NativeSelect.Field>
                              <NativeSelect.Indicator />
                            </NativeSelect.Root>
            ...
            <Dialog.Footer gap="3">
              ...
              <Button onClick={handleSave} loading={isLoading} disabled={isLoading}>
                {isLoading ? 'Menyimpan...' : 'Buat Pembelian'}
              </Button>
            </Dialog.Footer>
          </Dialog.Content>
        </Dialog.Positioner>
